...
class LinkGPTDataset(Dataset):
    ...
    def __init__(self, dataset_list: List[Dataset]):
        ...
        """
        # 过滤掉 None 数据集，确保 dataset_list 中的每个数据集都是有效的
        self.dataset_list = [ds for ds in dataset_list if ds is not None]
        if not self.dataset_list:
            raise ValueError("Dataset list cannot be empty or contain only None values.")
        
        self.dataset_lengths = [len(ds) for ds in self.dataset_list]
